...
| **Current story** | US-P08 queued — US-P07 readiness and keepalive complete |
...
| [US-P07](US-P07-readiness-and-keepalive.md) | `done` | Make readiness truthful and add an optional safe daily database probe. | Bounded 2.5s DB readiness check returning HTTP 503 on degradation/timeout; probe logging excluded from api_request_logs; asyncpg handshake timeout (5.0s) & pool recycle (1800s); protected /api/cron/supabase-keepalive route with timing-safe CRON_SECRET check and ENABLE_KEEPALIVE_CRON toggle; Vercel once-daily schedule (0 5 * * * UTC); 281 backend & 27 frontend tests passed; live production probes verified 200 OK. |
